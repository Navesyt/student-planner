...
import { rowsToAcademicItems, rowsToTimetableItems, type TableRow } from './groupPlanner';
...
async function readSpreadsheet(uri: string): Promise<TableRow[]> {
  ...
  workbook.SheetNames.forEach(name => {
    const values = XLSX.utils.sheet_to_json<unknown[]>(workbook.Sheets[name], { header:1, raw:false, defval:'' });
...
async function readPdf(uri: string): Promise<TableRow[]> {
  ...
  return rowsFromPdfText(await extractText(uri));
}

function rowsFromOcr(text: string): TableRow[] {
  return text.split(/\r?\n/).map((line,rowIndex) => ({ rowIndex, cells:line.split(/\s{2,}|\t|\|/).map(x=>x.trim()).filter(Boolean) })).filter(row=>row.cells.length>0);
}

async function pickDocument(): Promise<ImportedScope | null> {
...
export async function importScopeForStudent(groups: StudentGroups) {
  const scope = await pickDocument();
  if (!scope) return null;
  return { ...scope, items: rowsToAcademicItems(scope.rows, groups) };
}

export async function importTimetableDocument() {
  const timetable = await pickDocument();
  if (!timetable) return null;
  return { ...timetable, items: rowsToTimetableItems(timetable.rows) };
}

export async function scanTimetableImage() {
  const permission = await ImagePicker.requestCameraPermissionsAsync();
  if (!permission.granted) throw new Error('Autorisation caméra refusée.');
  const result = await ImagePicker.launchCameraAsync({ mediaTypes:['images'], quality:1 });
  if (result.canceled) return null;
  const asset = result.assets[0];
  const ocr = await recognizeText(asset.uri);
  const rows = rowsFromOcr(ocr.text);
  const document: ImportedDocument = { id:id(), name:'Emploi du temps scanné', format:'image', importedAt:new Date().toISOString() };
  return { document, rows, items:rowsToTimetableItems(rows) };
}
